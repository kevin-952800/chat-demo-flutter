...
class AvatarSelectPage extends StatefulWidget {
  static const String avatarFaceUrl =
  ...
  AvatarSelectPage(
      {Key? key, required this.controller, required this.selectedAvatarUrl})
      : super(key: key);
...
class AvatarSelectPageState extends State<AvatarSelectPage> {
  ...
  void initState() {
    ...
    for (int i = 0; i < AvatarSelectPage.avatarFaceCount; i++) {
      widget.avatarURLList.add(
          AvatarSelectPage.avatarFaceUrl.replaceAll("%s", (i + 1).toString()));
    }
  }
  ...
  _submitAvatar() async {
    if (widget.selectedAvatarUrl != "") {
      final result =
          await widget.controller?.updateAvatar(widget.selectedAvatarUrl);
      if (result?.code == 0) {
